...
EXPERT_TOPICS = [
    ...
    "Витамины группы В: зачем комплекс, а не одна таблетка",
    "Селен и щитовидка — что важно знать без паники",
    "Омега-3: как понять, хватает ли вам жирных кислот",
    "Пробиотики и микробиота: простыми словами для подписчиков",
    "Антиоксиданты в еде: мифы и реальная польза",
    "Почему «пить больше воды» не решает всё: электролиты коротко",
    "Кожа как зеркало дефицитов: что чаще упускают",
    "Спорт и восстановление: магний, калий, белок в рационе",
    "Беременность и грудное вскармливание: базовые нутриенты",
    "Пожилой возраст: чем поддержать кости и мышцы через питание",
]

# ─── Память недавних экспертных постов (понедельник) ─────────────────────────
EXPERT_RECENT_FILENAME = "expert_recent_snippets.json"
_MAX_EXPERT_RECENT = 8


def _expert_plain(html_text: str) -> str:
    import re
...
def expert_recent_load(state_path: str) -> list[str]:
    import json
    import os
...
def expert_recent_append(state_path: str, html_text: str) -> None:
    import json
    import os

    plain = _expert_plain(html_text)
    if len(plain) < 50:
        return
    prev = expert_recent_load(state_path)
    prev = [p for p in prev if p != plain]
    prev.insert(0, plain)
    prev = prev[:_MAX_EXPERT_RECENT]
...
def expert_recent_prompt_block(state_path: str) -> str:
    recent = expert_recent_load(state_path)
    if not recent:
        return ""
    chunks = []
    for s in recent[:6]:
        frag = s[:380] + ("…" if len(s) > 380 else "")
        chunks.append(f"«{frag}»")
    return (
        "\n\nНЕДАВНИЕ ЭКСПЕРТНЫЕ ПОСТЫ — не повторяй их приветствия, заголовки, порядок абзацев и формулировки:\n"
        + "\n---\n".join(chunks)
    )


def build_expert_user_prompt(topic: str, cta_note: str, state_path: str) -> str:
    """Промпт понедельника: разные заходы, приветствия и структура + анти-повтор по недавним постам."""
    import random

    recent_block = expert_recent_prompt_block(state_path)
    greet_modes = [
        "ПЕРЕД основным текстом: короткое живое приветствие или обращение (1 предложение). "
        "Запрещено начинать так же, как в недавних постах выше — другой тон: доброе утро по-новому, вопрос дня, "
        "узнаваемая бытовая ситуация, дружеское «разберём по полочкам» — выбери тип, который ещё не звучит как копия.",
        "ПЕРЕД заголовком: одно предложение — тёплое обращение без «Здравствуйте, дорогие друзья» и без канцелярита; "
        "каждый раз новая формулировка, не из списка штампов.",
        "Сначала строка с цепляющим заголовком в <b>...</b>, следующим абзацем — короткое приветствие или риторический вопрос к читателю; "
        "формулировки не должны повторять недавние посты.",
    ]
    greet = random.choice(greet_modes)

    anti = (
        "\n\nОБЯЗАТЕЛЬНО:\n"
        "- Каждый пост ощущается новой темой и новым углом: другие примеры, другой ритм, другой порядок мыслей.\n"
        "- Не копируй дословно приветствия, заголовки и связки из недавних постов ниже.\n"
        "- Избегай избитых заходов: «сегодня поговорим о», «как нутрициолог скажу», «ключ к здоровью» — если нужно похожее, сильно переформулируй.\n"
        "- Меняй манеру от первого лица: то «на приёме часто спрашивают», то короткий обезличенный кейс, то прямой совет без длинного вступления.\n"
    )
    if recent_block:
        anti += recent_block + "\n"
    ...
    if v == 1:
        body = (
            ...
            f"Тема: «{topic}»\n"
            f"{greet}\n"
            f"Структура: заголовок по теме в <b>...</b>; 3–4 абзаца пользы простым языком; финал — личный вывод врача (1–2 предложения).\n"
            f"Выдели жирным заголовок и 2–3 ключевых факта. 200–250 слов. {cta_note}"
        )
    elif v == 2:
        body = (
            f"Напиши экспертный пост для Perfect Organic от нутрициолога.\n"
            f"Тема: «{topic}»\n"
            f"{greet}\n"
            f"Формат «вопрос читателя → ответ эксперта»: сначала сформулируй короткий вопрос в духе подписчиков, "
            f"затем развернутый понятный ответ в 3–4 абзацах; заголовок в <b>...</b> должен отражать суть темы.\n"
            f"200–250 слов. {cta_note}"
        )
    else:
        body = (
            f"Напиши экспертный пост для Perfect Organic от лица врача-нутрициолога.\n"
            f"Тема: «{topic}»\n"
            f"{greet}\n"
            f"Структура: заголовок в <b>...</b>; блок «что важно знать» двумя абзацами; один абзац — практический совет без списка из 10 пунктов; "
            f"завершение — уверенный вывод без вопроса в конце.\n"
            f"200–250 слов. {cta_note}"
        )
    return body + anti
...
VIRAL_TOPICS = [
    ...
    # Доп. темы — реже повтор цикла, больше углов подачи
    "Почему вес стоит на месте при «правильном» питании — роль микроэлементов",
    "Сухая кожа и ломкие ногти: что часто упускают в рационе",
    "Сон пропал — не только стресс: что проверить в питании",
    "Утренний отёк лица и тяжесть: связь с водно-солевым балансом",
    "Тяга к сладкому вечером: не привычка, а сигнал организма",
    "Почему одних витаминов мало без «компании» минералов",
    "Зимняя апатия: чем поддержать себя через еду без таблеточного фанатизма",
    "Спортсмен или офис: кому чаще не хватает магния и калия",
    "Частые судороги в икрах ночью — что обычно забывают спросить у рациона",
    "Волосы редеют не только из-за шампуня: питание со стороны нутрициолога",
    "После 40 обмен «тормозит»? Мифы и реальность про метаболизм и микроэлементы",
    "Почему «пью витамины» а эффекта нет — 4 частые ошибки в еде",
    "Дети и подростки: чем отличаются потребности в минералах от взрослых",
    "Кофе и чай каждый день: что они вымывают и как компенсировать едой",
    "Анемия не только про железо: что ещё важно для энергии",
    "Почему витамин D зимой критичен даже если «я не выхожу на солнце»",
    "Щитовидка и йод: коротко и без паники для обычного человека",
    "Перепады настроения за неделю до цикла — питание как мягкая поддержка",
    "Зачем кальцию «помощники» и почему один продукт не спасёт кости",
    "Почему «суперфуды» не заменяют базу: простые продукты с максимумом пользы",
]
